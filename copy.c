             ...
             bool *last_write_made_hole)
{
  ...
  *total_n_read = 0;
  ...
  int progress_max = max_n_read;
  if (progress_max < 0) 
    {
      struct stat progress_stat_buf;
      if (fstat (src_fd, &progress_stat_buf) >= 0) 
        {
            progress_max = progress_stat_buf.st_size;
        }
    }
  ...
  while (max_n_read)
    {
      ...
      *total_n_read += n_read;
      ...
      prog_total += n_read;
      printf("\r%00d%", ((prog_total * 100) / progress_max));
//      printf("\nmax_n_read:\t%d\nprogress_max:\t%d\ntotal:\t%d\nprogress:\t%d\n",
  //            max_n_read, progress_max, total, progress);
      ...
      if (!make_hole)
        {
          ...
          if (full_write (dest_fd, buf, n) != n)
            {
              error (0, errno, _("\nerror writing %s"), quote (dst_name));
              return false;
            }
